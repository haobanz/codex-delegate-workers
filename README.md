...
**主代理始终沿用你在 Codex 中选择的模型和思考强度。** 主代理负责规划、拆分任务与最终验收；本项目只设置执行子代理，不覆盖主代理设置。

`0.7.0` 增加统一 MCP 接口：自定义模型 ID（例如 `deepseek/deepseek-v4.1-flash`）原样传给独立 `codex exec` 进程，沿用你现有的供应商和认证，主代理模型和思考强度保持不变。完整能力边界、工具表和路由判断见 [配置参考](docs/configuration.md#统一接口)。

接口采用一次性显式启用：安装或更新后运行一次 `dw interface enable`，再重启 Codex / 新建会话加载工具。普通安装和更新不会改动 `config.toml`；显式接口命令只变动本工具注册的那个 MCP 条目，卸载则会移除本工具拥有的匹配条目。完整生命周期见 [配置参考](docs/configuration.md#统一接口)。

全局执行预设只有一份来源：菜单、`dw configure`、预检和统一接口都读取已安装的 `workers.json`，不新增第二套 worker 设置。项目快照是可选的项目覆盖：`dw project init` 只把选定项目的执行快照写入该项目根目录的 `.delegate-workers-project.json` 和受管理的 `AGENTS.md` 规则段，已启用且规则块完整时优先于全局默认；不运行它就不会注册或影响任何项目。

新安装默认启用模型偏好：新 Codex 会话在分派工作时会读取执行预设，无需每次输入技能名。是否分派、任务怎么拆、用哪个模型、并行多少、是否重试或继续分派，都由主代理根据任务判断；用户明确指定的分工、模型、强度和范围优先。

原生派发、已启用的统一接口和本机已授权的本地 Codex CLI 三条执行路径及可用性判断见[派发路径与统一接口](#派发路径与统一接口)；worker 续接与恢复的模型一致性规范、项目 `tmp/` 规范、项目级 `AGENTS.md` 委派规则，以及无固定并发数、重试次数、升级链和任务记录的边界保持不变。
...
- 已安装并登录 Codex。执行沿用你现有的供应商与认证；设置和兼容性预检是本地静态操作，不发起模型请求，也不代表供应商认证成功。
- 原生子代理不是唯一执行途径：宿主能接受指定模型和强度时可以直接使用原生派发；不能接受时，可改用本工具已启用的统一 MCP 接口，或本机已有授权的本地 Codex CLI，两条路径都以显式模型和强度运行。
- 启用统一接口需要当前 Codex 支持 `codex mcp add` 注册本地 stdio MCP 服务器；注册成功不代表当前会话立刻可见，工具是否出现在上下文中仍取决于宿主加载与发现。
- 不需要另外配置 OpenAI API Key，也没有 Python 第三方依赖（统一接口只用 Python 标准库）。
...
2. 执行一键安装命令，再打开菜单或直接使用短命令。
3. 选择 **2. 设置执行模型和思考强度**（或 `dw configure`），常用模型预设为 `default`，输入执行模型 ID，再按数字选择中文标注的思考强度。直接回车可保留当前值。
4. 再次选择 **2** 可以设置备选预设 `complex`，也可以输入新的英文预设名称。提示“设为默认执行预设？”时输入“是”或“否”。
5. 如需统一接口，运行一次 `dw interface enable`（菜单中的第 **7** 项也可启用、查看或关闭接口），然后重启 Codex 或新建会话让工具生效。
6. 选择 **3. 查看状态和当前设置** 查看模型、版本、接口状态和本地代码修改状态。
7. 选择 **4. 开启 / 关闭默认委派** 检查开关。安装后默认开启。
8. 重新启动 Codex 会话，直接描述需求即可。命令行版需要退出并重新启动 `codex`；旧会话不会自动重读启动指令。

更新到新版、或已在使用统一接口时：

```bash
dw update            # 更新代码，保留执行设置和已启用的接口
dw interface enable  # 首次启用接口；已启用时是幂等的
dw interface status  # 只读查看注册状态
dw interface disable # 关闭本工具自己的 MCP 条目
```

`dw update` 会保留已启用接口及其安装路径，普通安装 / 更新不会改动 `config.toml`。启用、关闭接口和卸载时，只新增、更新或移除本工具注册的 `delegate-workers` MCP 条目；注册由本机 `codex mcp add` 写入，写入方可能重新格式化文件，但不会改动主模型、思考强度或其它无关设置，同名但非本工具管理的配置也不会被覆盖或删除。回滚到没有统一接口的旧版本前，需要先运行 `dw interface disable`，否则回滚会在改动前明确报错。接口注册本身不等于开启自动委派，`dw mode` 仍独立管理启动规则。
...
主代理在声称“无法派发”之前，应先检查当前启用的工具 schema / 清单和可用的文档化发现路径，并区分不同情况：派发工具不存在、接口拒绝指定模型、供应商/认证/网络错误、缺少运行时身份元数据、业务工具（表格、浏览器连接等）断开。这些情况互不推断，过期的运行时快照也不构成对某模型的账户级否定。

原生派发能接受当前选择和强度时，继续使用原生子代理。原生派发无法接受该组合时，先检查当前会话是否已启用本工具自己的 MCP 工具，并用 `list_models` 查看已配置偏好和当前生效来源（它只反映 `workers.json` 和项目快照，不是供应商完整模型目录）；这是比临时拼 CLI 命令更优先的通道。随后按当前用户或项目选择调用 `spawn_agent`，用 `get_agent` 轮询状态，必要时 `cancel_agent` 取消。

原生派发拒绝某个组合，只说明该宿主接口不接受它，不代表这个模型不可用：接口会把自定义模型 ID 原样转给独立 `codex exec` 进程，所以可以尝试用原来的 ID 通过 MCP 接口或本机 CLI 执行；实际能否成功取决于本机 CLI 版本和供应商是否支持该 ID 与强度，接口本身不据此保证执行，主会话模型和强度不变。

路由选择不是固定的重试或 fallback 链：`list_models`、兼容性预检和候选路径检查都是本地静态操作，不发起认证或模型请求。已经派发出去的任务失败时，先查看该 agent 的现有状态和失败原因，再决定是否新建派发；同一个供应商的失败不会因为改走直连 CLI 就自动变成成功，也不要求重试、替换或额外授权。原生派发在能接受所请求组合时仍然可用、也是允许的路径。

本工具 MCP 不可用时，本机已可用的 Codex CLI 仍是既有授权范围内可以尝试的执行途径：先核对 CLI 的真实 help 与功能，再显式传入所请求的模型和强度；能否成功取决于该 CLI 版本和供应商支持，不因换路径而保证执行。路径要具备任务需要的工具和文件，因为 CLI worker 可能没有主会话的业务连接。两条路径都不得静默替换模型或供应商、修改全局配置、索取 API Key，或只为执行而安装组件；也不得以本工具接口的名义修改宿主内建的 `collaboration.spawn_agent`。

接口 worker 是独立进程，不是原生 agent。工具返回的 `agent_id` 是本接口自己的 ID，与后端 CLI 会话 ID 分开报告；会话头部和成功响应只说明记录的配置与路径可用，都不证明上游物理模型身份。取消会先真正停止子进程树再报告终态；MCP 服务器断开时，它拥有的运行中任务会被取消；服务器重启不会自动恢复任务。输出和日志只写在项目根的 `tmp/delegate-worker-*` 下。

当明确分工或项目规则要求特定模型/强度、而没有任何已授权途径可执行时，只暂停被阻塞的执行并说明具体限制、询问替代方案；主代理不得以“只读调查”“重试”或“fallback”等名义代做被分配的业务任务，披露限制不等于授权接手。需要保存的提示、日志和输出文件放在项目 `tmp/`。工具表、`spawn_agent` 最小示例和完整边界见[配置参考](docs/configuration.md#统一接口)。
...
将这行加入 Bash 的 `~/.bashrc` 或 Zsh 的 `~/.zshrc` 可使新终端也生效。安装器不修改 Shell 启动文件，也不会覆盖已有的同名程序；普通安装 / 更新不会改动 `config.toml`，开启默认委派时只在全局指令文件中维护本项目的标记段落。只有显式运行 `dw interface enable` / `dw interface disable`，或运行 `dw uninstall` 移除本工具条目时，才会变动本工具注册的那个 MCP 条目；写入由本机 `codex mcp add` / 移除完成，写入方可能重新格式化 `config.toml`，但主模型、思考强度和其它无关设置保持不变。
...
统一接口的标记文件先在项目 `tmp/` 暂存，再替换到目标位置。当目标标记位于另一个磁盘或文件系统时，`os.replace` 会以 `EXDEV`（Windows：`WinError 17`）失败；此时工具直接写入最终标记，不再依赖跨盘原子替换。普通写入失败会恢复旧内容（新建的标记则删除），但这种方式不承诺掉电时的原子性。
